...
exports.signUp = async(req, res, next) => {
    ...
    try {
        ...
        return res
            .status(400)
            .send({ error: "User not created successfully!", error: err });
    }
};
...
exports.logIn = async(req, res) => {
    ...
    if (error) return res.status(400).send({message : error.details[0].message , status : 100});
    ...
    if (!foundUser)
        return res.status(400).send({ message: "invalid Email or Password" , status : 100 });
    ...
    try {
        ...
        return res
            ...
            .send({ "auth-token": token, userId: foundUser._id , status : status });
    } catch (error) {
